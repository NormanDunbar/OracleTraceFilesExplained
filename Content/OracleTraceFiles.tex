...
\begin{lstlisting}[numbers=none,caption={Trace File Header}]
...
\begin{note}
Remember, when Oracle writes a line to the trace file, it is done at the \emph{end} of the process that was indicated by the written line. For example, if you see a \inline{FETCH} line in the trace file, that was written at the tim (\inline{tim}) that the \inline{FETCH} completed. The trace file should be considered a list of things \emph{that happened} and not \emph{things that are about to happen}.
\end{note}
...
Under normal circumstances, a statement that is parsed (executed etc) at \inline{dep=n} has been called recursively, to facilitate a statement, that will follow in the trace file, that is itself parsed (executed etc) at \inline{dep=n-1}.

\subsubsection{Waits}\label{waits}\index{Waits}

\inline{WAIT} lines in a trace file are similar, in that the
\inline{WAIT} must complete, and so is written to the trace file
\emph{before} the statement that incurred the wait. For example, a
\inline{FETCH} that had to wait for
\inline{db file scattered read} events, will appear later in the
trace file that the individual \inline{WAIT} lines that the
\inline{FETCH} suffered from.
...
tim\strut
\end{minipage} & \begin{minipage}[t]{0.65\columnwidth}\raggedright\strut
Time, in microseconds, for this statement to be parsed. This is not the
time it took!\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.14\columnwidth}\raggedright\strut
hv\strut
...
The \inline{lid} field is unknown at present. It's probably an ID of some kind, but for what?
...
\section{PARSE}\label{parse}\traceline{PARSE}

Normally, after the \inline{PARSING IN CURSOR} lines, you will see a \inline{PARSE} line for the same cursor ID. This is not always the case, for example, if you started the trace after the statement had been parsed, Oracle will write the \inline{PARSING IN CURSOR} lines, but not the \inline{PARSE}, to the trace file.
...
\tightlist
\item
  Physical reads occur when Oracle must read a block, from disc, into   the buffer cache. This indicates also an additional \inline{cr} as   the block will be used unchanged.
\item
...
\subsection{PARSE ERROR}\label{parse-error}\traceline{PARSE ERROR}
...
\section{EXEC}\label{exec}\traceline{EXEC}
...
tim\strut
\end{minipage} & \begin{minipage}[t]{0.65\columnwidth}\raggedright\strut
Time, in microseconds, for this statement to be executed. This is
\textbf{not} the time it took!\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
You may wish to refer back to the details on the \inline{PARSE} line, above, for details of the \inline{p}, \inline{cr} and \inline{cu} block statistics in the \inline{EXEC} line.

\section{FETCH}\label{fetch}\traceline{FETCH}
...
\section{WAIT}\label{wait}\traceline{WAIT}
...
The \inline{p1}, \inline{p2} and \inline{p3} parameters are different depending on the wait event name. You may require to loom them up in \inline{V\$EVENT\_NAME} to understand what they refer to. For the event above, we see the following:
...
\section{ERROR}\label{error}\traceline{ERROR}
...
\section{STAT}\label{stat}\traceline{STAT}
...
The \inline{STAT} lines show you \emph{exactly} how Oracle went about getting the data back to you. It is possible for this to be different for the plan displayed by the \inline{EXPLAIN PLAN FOR ...} statement. In a trace file, it shows what happened, as opposed to what \emph{might} happen!
...
\begin{longtable}[]{@{}l|l@{}}
\hline
\caption{Stat - Fields for Oracl;e 10g Onwards\ldots{}\textit{continues on next page}}
\endfoot
...
size\strut
\end{minipage} & \begin{minipage}[t]{0.65\columnwidth}\raggedright\strut
Form 11g, an estimate of the size, in bytes, of the data returned by
this step.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.14\columnwidth}\raggedright\strut
card\strut
...
\section{CLOSE}\label{close}\traceline{CLOSE}
...
\item
  ...
  \item
    \inline{SESSION\_CACHED\_CURSORS} is set to zero, and no caching is permitted.
  \item
...
\section{XCTEND}\label{xctend}\traceline{XCTEND}
...
\section{BINDS}\label{binds}\traceline{BINDS}
...
\item
  The \inline{value} can be easily read form the trace file. Had this been in a different character set, \inline{ALUTF16} for example (\inline{csi=2000}), then the value would most likely have been dumped in hexadecimal, as follows:
...
A \inline{REF\_CURSOR} bind, will resemble the following. It has an \inline{oacdty} of 102, which appears to confuse the \inline{value} field a little, as it implies that it cannot handle a data type of 102. Hmmm.
...
\section{UNMAP}\label{unmap}\traceline{UNMAP}
